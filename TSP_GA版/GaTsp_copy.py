...
class Species:
    ...
    def _prepare(self, *args, **kwargs):
        ...
        self._distance_map = np.zeros((self.N, self.N), dtype=np.float64)
...
class Salesman:
    ...
    def __repr__(self):
        # print(f"経路長は{self.length}だよ！！")
        # print(f"len変更前:{lenlenlength}")
        # self.lenlenlength = self.length
        # print(f"len変更後:{lenlenlength}")
        
        global lenlenlength
        # print(f"変更前self.length:{self.length},lenlenlength:{lenlenlength}")
        lenlenlength = self.length
        # print(f"変更後self.length:{self.length},lenlenlength:{lenlenlength}")
    ...
    def picked_indeces(self):
        if self._picked_indeces is None:  # encode path to picked_indeces.
            bits = np.ones(self.species.N, dtype=int)
    ...
    def sample_betters(salesmans, k:int, nelites:int=0, *args, **kwargs):
        ...
        # print(f"k:{k},nelites:{nelites},k - nelites:{k - nelites}")
...
class Model:
    def __init__(self, species:Species, max_population:int=None, breedrates:float=None, *args, **kwargs):
        self.species = species
        ...
        self.breed_rates = {
            Breed.ScrambleMutation: breedrates,
            Breed.TranslocationMutation: breedrates,
            Breed.SwapAdjacentPoints: breedrates,
            Breed.SwapMutation: breedrates,
            Breed.InversionMutation: breedrates,
            Breed.CyclicCrossover: breedrates,
            Breed.PartialCrossover: breedrates,
            Breed.OrderCrossover: breedrates,
            # Breed.OrderBasedCrossover: 0.6,
            Breed.PositionBasedCrossover: breedrates,
            Breed.SinglePtCrossover: breedrates,
            Breed.TwoPtsCrossover: breedrates,
            Breed.MultiPtsCrossover: breedrates,
            Breed.UniformCrossover: breedrates,
            Breed.SubstitutionMutation: breedrates,
        }
    ...
    def _evoluate(self, epoch, *args, **kwargs):
        ...
        # print(f"self.max_population:{self.max_population},epoch * self.species.N // 10:{epoch * self.species.N // 10},nparents:{nparents}")
...
class Logger_trace(Logger):
    ...
    def log_end(self, model:Model, *args, **kwargs):
        ...
        if self._level > 0:
            ...
            global epoepoepoch
            global tititime
            epoepoepoch = model.leader_changed_epoch
            tititime = et - self._st
    
    # def extract_epoch(self, model:Model, *args, **kwargs):
    #     return model.leader_changed_epoch
    
    # def extract_time(self, model:Model, *args, **kwargs):
    #     et = datetime.now()
    #     return et - self._st
