from week6_haruo import read_input, cal_dist, optimal_tour, calculate_total_distance, format_tour, distance
...
class Baseline_first:
    def __init__(self, file_number:int, first_point:int=0 ,baseline_tour:list=None, dist:list=None):
    ...
    def calculate_baseline_tour(self, cities, dist):
        res_tour = optimal_tour(dist, cities, self.first_point)
        # res_tour = solve(dist, cities, self.first_point)
    ...
    def read_city_information(self):
        ...
        baseline_tour,distance = self.calculate_baseline_tour(cities, dist)
...
def plot_heatmap(coordinates, values, n):
    # Extract x and y coordinates from the list of 2D coordinates
    y_coords, x_coords = zip(*coordinates)
    before_y = list(y_coords)
    after_y = [1.0 - y for y in before_y]
    y_coords = tuple(after_y)
    ...
    # Add values as annotations near each point
    for x, y, val in zip(x_coords, y_coords, values):
        plt.annotate(f'{val:.2f}', (x, y), textcoords="offset points", xytext=(0, 0.5), ha='center', fontsize=6, color='black')

    if n >= 50:
        denominator = 51
    for position in np.arange(0.0, 1.0000000000000001, 1/(denominator-1)):
        plt.axvline(x=position, color='black', linestyle='--', linewidth=0.5)
        plt.axhline(y=position, color='black', linestyle='--', linewidth=0.5)
    plt.xticks(np.arange(0.0, 1.01, 1/16))
    plt.yticks(np.arange(0.0, 1.01, 1/16))
    plt.show()
...
if __name__ == '__main__':
    ...
    top3 = dodo.search()
    top1_length = Search_in_same_baseline(top3[0]).search_all()[0].length
