from week6_3d import read_input, cal_dist, optimal_tour, calculate_total_distance, format_tour, distance, cal_shortpath, read_atsp_file
...
class Swap:
    ...
    def swap_and_distance(self, random_numbers:list):
        ...
        #     print(f"baseline_tour_copy:{baseline_tour_copy}, new_tour:{new_tour}")
        #     print(f"remove_length:{remove_length}, remove_position:{remove_position}, insert_position:{insert_position}")
        #     print(f"insert_before:{insert_before}, insert_after:{insert_after}, insert_first:{insert_first}, insert_last:{insert_last}")
        #     print(f"cut1:{cut1}, cut2:{cut2}, cut3:{cut3}, connect1:{connect1}, connect2:{connect2}, connect3:{connect3}")
        #     raise ValueError("Error: new_distance is 37.0")
        # time5 = time.time()
...
class Different_first_baseline:
    def __init__(self, dist:list, short_path:list):
    ...
    def search(self):
        ...
        for start in range(search_city_num):
            baseline = Baseline_first(self.dist, self.short_path, start)
            ...
            print(f"経過時間：{end_time - start_time:.5f}秒, 予想残り時間：{((end_time - start_time)/(start+1)) *(search_city_num-start-1):.1f}秒")
        return tours_all
...
if __name__ == '__main__':
    ...
    dodo = Different_first_baseline(dist, short_path)
    top3 = dodo.search()
    last_1 = top3[0]
    ...
    for i in range(100):
        ...
        print(f"経過時間：{laptime:.5f}秒")

    for tour in retry_top:
        print(tour.length)
        # ooooo = tour.order
        # print(ooooo)
        # for ssett in range(len(ooooo)-1):
        #     print(dist[ooooo[ssett]][ooooo[ssett+1]])
        # print(dist[ooooo[-1]][ooooo[0]])
